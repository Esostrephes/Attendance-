...
async def index():
    """Serve the main page - HTML is embedded here, no external file needed"""
    return """
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    <div class="container">
        ...
        <div class="instruction">
            <p><strong>Instructions:</strong> Enter your details, click "Start Enrollment" and slowly turn your head left and right. The system will capture different angles and save to Firebase.</p>
        </div>
        ...
        <div class="controls">
            ...
            <button id="viewBtn">View All Users</button>
        </div>
    ...
    <script>
        ...
        startBtn.addEventListener('click', async () => {
            try {
                ...
                const response = await fetch('/start_enrollment', {
                    method: 'POST',
                ...
                if (data.status === 'success') {
                    ...
                    userIdText.textContent = currentUserId.substring(0, 8) + '...';
            ...
            try {
                ...
                if (data.status === 'success') {
                    ...
                    if (data.users.length === 0) {
                        usersContainer.innerHTML = '<p>No users enrolled yet.</p>';
                    } else {
                        data.users.forEach(user => {
                            const card = document.createElement('div');
                            card.className = 'user-card';
                            card.innerHTML = `
                                <h4>${user.user_name || 'Anonymous'}</h4>
                                <p><strong>ID:</strong> ${user.user_id}</p>
                                ${user.user_email ? `<p><strong>Email:</strong> ${user.user_email}</p>` : ''}
                                <p><strong>Samples:</strong> ${user.samples_count || 0}</p>
                            `;
        ...
        async function processFrame() {
            ...
            try {
                ...
                if (data.is_complete) {
                    ...
                    message.textContent = '✅ Enrollment complete and saved to Firebase!';
                }
